...
static int32_t setNavigationRaw()
{
	...
	if (homeLocation.Set == HOMELOCATION_SET_FALSE) {
		...
		get_linearized_3D_transformation(gpsPosition.Latitude,  gpsPosition.Longitude, gpsPosition.Altitude,
		                                 homeLocation.Longitude, homeLocation.Latitude,  homeLocation.Altitude,
		                                 T, NED);
...
static int32_t updateAttitudeINSGPS(bool first_run, bool outdoor_mode)
{
	...
	bool gps_init_usable = gps_updated && (gpsPosition.Satellites >= 7) && (gpsPosition.PDOP <= 3.5f) && (homeLocation.Set == HOMELOCATION_SET_TRUE);
	...
	} else if (outdoor_mode && (gpsPosition.Satellites < 6 || gpsPosition.PDOP > 4.0f)) {
		if (gpsPosition.Satellites < 6)
			set_state_estimation_error(SYSTEMALARMS_STATEESTIMATION_TOOFEWSATELLITES);
		else if (gpsPosition.PDOP > 4.0f)
			set_state_estimation_error(SYSTEMALARMS_STATEESTIMATION_PDOPTOOHIGH);
	...
	gps_updated &= (gpsPosition.Satellites >= 6) && (gpsPosition.PDOP <= 4.0f) && (homeLocation.Set == HOMELOCATION_SET_TRUE);
